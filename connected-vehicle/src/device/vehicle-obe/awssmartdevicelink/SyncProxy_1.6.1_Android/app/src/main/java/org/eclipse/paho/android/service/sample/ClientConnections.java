...
public class ClientConnections extends ListActivity {
  ...
  private void connectAction(Bundle data) {
    ...
    String keystore_pass = (String) data.get(ActivityConstants.keystore_pass);
    ...
    if (ssl){
        try {
        	...
        	{
              FileInputStream keystore = new FileInputStream(ssl_key);
              KeyStore keystoreKeys = KeyStore.getInstance("BKS");        // Bouncy Castle
              keystoreKeys.load(keystore, keystore_pass.toCharArray());

              KeyManagerFactory keyManagerFactory = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());
              keyManagerFactory.init(keystoreKeys, keystore_pass.toCharArray());
