...
exports.handler = function(event, context) {
  ...
  var endtime = moment.utc().minutes(moment.utc().minutes()+1).format("MM/DD/YYYY hh:mm:ss");

  //set incidents id to vehicle VIN to avoid duplicate
  var uuid = event.state.reported.vin;

  var xmloutput=
    '<TRAFFICML_INCIDENTS EXTENDED_COUNTRY_CODE="A0" VERSION="2.2">\n'
    + '  <TRAFFIC_ITEMS>\n'
    + '    <TRAFFIC_ITEM>\n'
    + '      <ORIGINAL_TRAFFIC_ITEM_ID>'+uuid+'</ORIGINAL_TRAFFIC_ITEM_ID>\n'
